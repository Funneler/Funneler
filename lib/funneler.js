...
module.exports = class Funneler {
    ...
    emit(id) {
        let p = this.storage.emit.apply(this.storage, [id]);
        this.promises.push(p);
        return p;
    }

    reduce(id) {
        let p = this.storage.reduce.apply(this.storage, [id]);
        this.promises.push(p);
        return p;
    }

    slice(offset, limit) {
        let p = this.storage.slice.apply(this.storage, [offset, limit]);
        this.promises.push(p);
        return p;
    }

    data(item, itemOrIndex, dataOrEmpty) {
        let p = this.storage.data.apply(this.storage, [item, itemOrIndex, dataOrEmpty]);
        this.promises.push(p);
        return p;
    }
